...
class MelUnitDataset(Dataset):
    ...
    Preprocessed data (log-mel-spectrogram & unit series) should be pre-generated and paths should be stored in metadata.
    """
    ...
    def __init__(self, root: Path, train: bool = True, discrete: bool = False, upsample: bool = True, size_codebook: int = 1):
        ...
        fname_meta = "train.txt" if train else "validation.txt"
        with open(root / fname_meta) as file:
    ...
    def __getitem__(self, index: int):
        """
        Load -> Clipping -> Tensor -> head padding -> cast
        ...
        """
        # Item paths
        rel_path = self.metadata[index]
        path_mspc_series = (self.mels_dir  / rel_path).with_suffix(".npy")
        path_unit_series = (self.units_dir / rel_path).with_suffix(".npy")

        # Load
        mspc_series: NDArray = np.load(path_mspc_series).T
        unit_series: NDArray = np.load(path_unit_series)

        # Clipping - adjust to short one
        ...
        # Head padding - Zero padding of time for AR input :: (T_mspc = 2*T_unit, Freq) -> (T_mspc = 2*T_unit+1, Freq)
    ...
    def pad_collate(self, batch):
        ...
        # Special flag value "zero" is `size_codebook` if discrete
        units = pad_sequence(
